...
def parse_args():
    ...
    parser.add_argument(
        "--index_path", type=str, help="Path to the folder where the index was built"
    )
...
def get_user_query():
    query_dict = {
        k: None for k in ["paper_field", "date", "title", "authors", "abstract"]
    }
            ...
            with idx.searcher(weighting=scoring.TF_IDF()) as searcher:
                ...
                if len(hits) == 0:
                    ...
                    for i, hit in enumerate(hits, start=1):
                        ...
                        d["keywords"] = ", ".join(
                            get_keywords(hit["pdf"], n=10, only_words=True)
                        )
                        d["highlights"] = {}
